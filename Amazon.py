...
def title_small(s):
    s = s.lower()
    return s
def amazon(mubiao_file,source_file,CiBiao,account,TG_VP,price,shipping,leixing,style,fangfa,color_num,zuhe,zibiao):
    # 读取源表格文件
    # 替换为实际的源文件路径
    ...
    # df_source.replace('*192.3.95.71*', 'wang.yant88.xyz', inplace=True)
    #提取图片
    ...
    if zuhe == 1:
        img_zuhe_1 = df_source.iloc[0:,28]
    #     最后一张是尺码表
    ...
    if (account == "lc"):
        main_img = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in main_img]
        main_img_main = main_img_main.replace("192.3.95.71", "wang.yant88.xyz")
        final_img = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in final_img]
        img_1 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_1]
        img_2 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_2]
        img_3 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_3]
        img_4 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_4]
        img_5 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_5]
        img_6 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_6]
        img_7 = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_7]
        if zuhe == 1:
            img_zuhe = [url.replace("192.3.95.71", "wang.yant88.xyz") if not pd.isna(url) else url for url in img_zuhe]
    elif account == "YOULE":
    ...
    if zibiao == 1:
        ...
        new_title = pinjie.zibiao(length,CiBiao,title)
    ...
    df_new['sport1'] = np.nan
    df_new['sport2'] = np.nan
    ...
    #  point5 = random.sample(point5, len(point5))
    #     # 首先获取Series的索引
    #     indices = list(range(len(point5)))
    #
    #     # 使用random.shuffle打乱索引
    #     random.shuffle(indices)
    #
    #     # 使用乱序的索引重新排列Series
    #     point5 = point5.iloc[indices]
    ...
    df_new['fur_description'] = [p5[1]]*length
    if feed_product == 'pants':
        df_new['fabric_type'] = [p5[2]]*length
    else:
        if 'Linen' in string:
    ...
    if leixing == 'pants' or leixing == 'jumpsuit' or leixing == 'shorts' or leixing == 'romper':
    ...
    file_path = os.path.join(r'C:\Users\123\PycharmProjects\pythonProject',output_filename)
    utils.copy_file(mubiao_file,file_path)
